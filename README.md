...
- Saved campaigns, agent runs, decisions, and creative assets in InsForge
...
| System of record | InsForge/Postgres and Storage | Campaigns, agent runs, messages, approvals, and durable assets |
...
The application currently uses one seeded `bilads` workspace and a server-only InsForge admin client. It does not provide end-user accounts or tenant isolation.

## Prerequisites

- Node.js 20.9 or newer and npm
- An InsForge project for the full campaign workflow
...
2. Create the local environment file and fill in the required values:
   ...
   At minimum, configure `INSFORGE_BASE_URL`, `INSFORGE_API_KEY`, and `GMI_API_KEY`. The default GMI endpoints and model IDs are already listed in `.env.example`.

3. Link the repository to your InsForge project if it is not already linked, then provision the schema and private storage buckets:
   ...
   npx @insforge/cli storage create-bucket generated-creatives --private
   npx @insforge/cli config plan --file insforge.toml
   npx @insforge/cli config apply --file insforge.toml --auto-approve
   ```

   Skip bucket creation when those buckets already exist. The migration seeds the single Bilads workspace used by the server repository.
...
| --- | --- | --- |
| InsForge | `INSFORGE_BASE_URL`, `INSFORGE_API_KEY`, `INSFORGE_PROJECT_ID` | Base URL and API key are required at runtime; project ID is used by CLI/infrastructure workflows |
...
`INSFORGE_API_KEY` is a full-access admin credential and must never be exposed through a `NEXT_PUBLIC_*` variable. `BILADS_APP_ORIGIN` shapes same-origin browser requests but is not user authentication; restrict access at the deployment layer if the shared workspace should not be public.
...
Live integrations intentionally fall back where possible, but InsForge is required for the current create-and-save campaign path. Generated performance figures are planning scenarios, not guaranteed outcomes.
